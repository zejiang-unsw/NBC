...
#-------------------------------------------------------------------------------------------------
#' Function: Calculate rainfall statistics
#'
#' @param n     The number of columns (e.g., stations)
#' @param y     A matrix of time series rainfall data
#'
#' @return
#' @export
#'
...
rain.stats <- function(n,y)
{
	...
	# Aggregate to annual
    z.hat <- aggregate(y,nfrequency=1)                                                    # Create annual rainfall time series
...
#-------------------------------------------------------------------------------------------------
#' Function: Nested bias correction for calibration - rainfall
#'
#' @param n           The number of columns (e.g., stations)
#' @param y           A matrix of time series rainfall data
...
#' @param tol         Tolerance for small values, default 0.1 for rainfall
#'
#' @return
#' @export
#'
#' @examples
#'
nest.rain.cal <- function(n,y,obs.sta,start.yr,end.yr,tol=0.1)
{
  ...
  #-----------------------------------------------------------------------------------------------------------correction factor
    ...
    month.const[month.const < 0] <- 0              # If monthly total is less than zero, make 0.
    ...
    year.const[year.const < 0] <- 0                # If yearly total is less than zero, make 0.
...
#-------------------------------------------------------------------------------------------------
#' Function: Nested bias correction for validation - rainfall
#'
#' @param n           The number of columns (e.g., stations)
#' @param y           A matrix of time series rainfall data
#' @param obs.sta     Observed (Target) data statistics
...
#' @examples
#'
nest.rain.val <- function(n,y,obs.sta,mod.sta,start.yr,end.yr,tol=0.1)
{
  obs.mon.mean=obs.sta$mon.mean;obs.mon.sd=obs.sta$mon.sd;obs.mon.cor=obs.sta$mon.rho
  obs.yr.mean=obs.sta$yr.mean;obs.yr.sd=obs.sta$yr.sd;obs.yr.cor=obs.sta$yr.rho
  ...
  nyrs <- end.yr +1 - start.yr									                 # Number of years
    ...
    # Unpack by scaling with observed annual mean and standard deviation
    z.tick <- t(t(z.tick)*obs.yr.sd+obs.yr.mean)
    z.tick <- ts(z.tick,start=start.yr,end=end.yr)                                  # Make into time series
